...
import Data.THUnify.Prelude (expandType, HasMessageInfo(..), message, pprint1, toName)
...
-- A type variable is phantom if we never encounter it during the
-- traversal of the type's declaration.  This returns a list of the
-- non-phantom variables in a type.
nonPhantom :: DsMonad m => Name -> m [Type]
nonPhantom tname =
{-
    runQ (foldType go) (ConT tname) []
    where
      go :: [Con] -> [Type] -> Q [Type]
      go
-}
